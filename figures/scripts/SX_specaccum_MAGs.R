...
# read in manually curated MAG metaG coverm mean stats (>=90% ANI, >=75% hcov)
data <- read.csv('/fs/ess/PAS1117/riddell26/Grantham_Bioreactor/02-get-relative-abundance/results/meancov/metaG/MAGs/metaG_MAG_mean_97_75_75.tsv', sep='\t')
...
for (col in 1:ncol(data)) {
  colnames(data)[col] <- sub("_97FILTERED_SORTED\\.Mean|_MG.*", "", colnames(data)[col])
}
...
ggplot() +
  ...
  labs(x = "Samples", y = "MAGs (cumulative)") +
...
ggsave("S2_MAG_specaccum_metaG.pdf", width = 8, height = 6, dpi = 300)
...
data <- read_excel('/fs/ess/PAS1117/riddell26/Grantham_Bioreactor/02-get-relative-abundance/results/MAGs/metaT/MAG_abundance_table.xlsx')
data <- data.frame(data[,-40])
...
ggsave("SX_MAG_specaccum_metaT.pdf", width = 8, height = 6, dpi = 300)
